...
- **Two backends, one sequence**: the per-frame math that decides what the frame LOOKS like lives once, in `frame.ts` (derived timeline, the segment build, line + glow uniforms, the bloom energy filter) — retune it there and both backends follow. What is still written twice in `effect.ts` and `effect-gl.ts` is the state machine (play/move/dock), the mosaic and dot uniform values, and all pass encoding: change one and you MUST change the other, or WebGPU and WebGL2 visitors see different animations on the same homepage. Same rule for `shaders/*.ts` ↔ `shaders/gl.ts`. Diffing the two `frame()` bodies with comments stripped is the fastest drift check — what's left should reduce to API calls plus the WebGPU-only shader-mode blocks.
